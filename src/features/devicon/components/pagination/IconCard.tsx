...
const IconCard = ({ icon, deviconBranch, onSelect }: IconCardProps) => {
    ...
    return (
        <button onClick={()=>{onSelect(icon)}} aria-label={`Select icon: ${icon.name}`} className="overflow-hidden bg-white shadow-sm hover:bg-green-500 hover:text-white text-slate-400 flex  flex-col justify-center align-middle h-32 text-center rounded-lg w-full">
