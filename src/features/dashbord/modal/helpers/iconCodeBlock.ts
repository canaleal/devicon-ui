...
export const createIconCodeBlockText = async (
  ...
  const codeClockFormats: Record<
    CodeBlockOptionTypes,
    () => string | Promise<string>
  > = {
    Link: () => iconUrl,
    '<img> Tag': () => getImageTag(icon, iconUrl, iconSize),
    SVG: async () => adjustSVGAttributes(await fetchIconSVG(iconUrl), iconSize),
    '<i> Tag': () => getITag(icon, selectedVersion),
  };

  return codeClockFormats[selectedCodeBlockFormat]
    ? await codeClockFormats[selectedCodeBlockFormat]()
    : '';
};
