...
export const Tooltip = ({ children, content, position, flashMessage }: TooltipProps) => {
    ...
    const handleClick = () => {
        if (flashMessage) {
            ...
            timeoutId = window.setTimeout(() => { // Use window.setTimeout for clarity, though it's optional
    ...
    return (
        ...
        >
            ...
            {flashVisible && (
                <div className={`absolute z-10 px-2 py-1 bg-primary-600 text-white rounded max-w-xs break-words ${positionClasses[position]}`}>
                    <p>{flashMessage}</p>
                </div>
            )}
            {children}
