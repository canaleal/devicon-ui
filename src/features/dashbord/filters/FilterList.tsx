...
export const FilterList = ({ filterGroup, iconMap, hasMaxHeight, handleFilter, resetFilterGroup }: FilterListProps) => {
  const numberOfActiveFilters = filterGroup.filters.reduce((count, filter) => (filter.isSelected ? count + 1 : count), 0);

  return (
    <div className="flex flex-col text-gray-800 dark:text-gray-200">
      <FilterHeader categoryName={filterGroup.filterType} numberOfActiveFilters={numberOfActiveFilters} totalFilters={filterGroup.filters.length} resetFilterGroup={() => resetFilterGroup(filterGroup)} />
