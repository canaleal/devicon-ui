    ...
    return (
        <div className="flex flex-row dark:text-white">
            ...
            {pagesToRender.map((page, idx) => {
                ...
                return isNumber ? (
                    <button key={key} className={`px-4 py-2 rounded-md text-sm ${page === currentPage ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 hover:shadow-sm dark:hover:bg-zinc-1000'}`} onClick={() => setCurrentPage(page)}>
                        {page}
                    </button>
                ) : (
