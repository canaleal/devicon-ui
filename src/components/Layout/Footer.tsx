...
export const Footer = () => {
  ...
  return (
    <section className='flex flex-col md:flex-row gap-16 px-6 md:px-8 2xl:px-16 py-16 bg-dark-900 text-white border-t border-dark-500'>
      ...
      <div className='flex-1 flex flex-col gap-4'>
        ...
        {PROJECT_LINKS.map((link) => (
          <a key={link.name} href={link.url} target='_blank' className='hover:text-primary-600 text-sm w-fit'>
            {link.name}
          </a>
        ))}
      </div>
